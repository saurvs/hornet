...
impl Client {
    ...
    fn split_mmap_views(&self, mmap_view: MmapViewSync, wi: &MMVWriterInfo, metrics: &mut [&mut MMVMetric]) -> io::Result<()> {
        ...
        for (i, m) in metrics.iter_mut().filter(|m| m.type_code() == metric::STRING_METRIC_TYPE_CODE).enumerate() {
...
fn test_metric() {
    //use std::fs::File;
    use metric::{Metric, Semamtics};

    let mut trials = Metric::new(
        "trials", 1, Semamtics::Counter, 0,
        "hwllo world".to_owned(), // init_val
        "Trials",
        "Number of Monte Carlo trials");
    let mut pi = Metric::new(
        "pi", 1, Semamtics::Instant, 0,
        0.0, // init_val
        "Estimated Pi",
        "Estimated value of Pi through Monte Carlo trials");

        let mut trials2 = Metric::new(
        "tdsdrials", 1, Semamtics::Counter, 0,
        "fdsfdsa world".to_owned(), // init_val
        "fdsafda",
        "fda of fdsfd Carlo trials");

    let mut pi2 = Metric::new(
        "pi", 1, Semamtics::Instant, 0,
        0.0, // init_val
        "3Estimated Pi",
        "3Estimated value of Pi through Monte Carlo trials");

    let client = Client::new("metric").unwrap();
    client.export(&mut [&mut trials, &mut pi, &mut trials2, &mut pi2]).unwrap();

    pi.set_val(3.14).unwrap();
    trials.set_val("FROOOOZEN".to_owned()).unwrap();
    trials2.set_val("LUUUUUKKEEE".to_owned()).unwrap();
    pi2.set_val(3.33333333).unwrap();
}
